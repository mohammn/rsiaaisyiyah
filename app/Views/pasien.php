                ...
                <table class="table" id="tabelPasien">
                    ...
                    <tbody>
                        ...
                        for ($i = 0; $i < count($data->pasien); $i++) {
                            ...
                            echo '<td>';
                            echo '<button class="btn-estetik btn-sm-estetik bg-vibrant-purple" onclick="lihatPj(\'' . esc($data->pasien[$i]["no_rkm_medis"], 'js') . '\', \'' . esc($data->pasien[$i]["nm_pasien"], 'js') . '\', \'' . esc($data->pasien[$i]["namakeluarga"], 'js') . '\', \'' . esc($data->pasien[$i]["alamatpj"] . ', Desa/kel. ' . $data->pasien[$i]["kelurahanpj"] . ', Kec. ' . $data->pasien[$i]["kecamatanpj"] . ', ' . $data->pasien[$i]["kabupatenpj"], 'js') . '\')"><i class="fas fa-search"></i> Lihat PJ</button>';
                            echo ' <a href="' . base_url("/pasien/gelang/" . $data->pasien[$i]["no_rkm_medis"])  . '" target="_blank" class="btn-estetik btn-sm-estetik bg-vibrant-blue"><i class="fas fa-ring"></i> Gelang</a>';
                            echo '</td>';
                            echo "</tr>";
                        }
                        ?>
