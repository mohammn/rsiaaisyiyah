            ...
            <div class="subpage">
                ...
                <div class="row">
                    <div class="col-12 text-center">
                        <p style="font-size: 14pt; margin:10px;" class="text-uppercase fw-bold">DAFTAR PENYIMPANAN BARANG MILIK PASIEN
                        </p>
                    </div>
                </div>
                ...
                <table class="table table-sm table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th colspan="2" class="text-center align-middle">Kondisi Barang</th>
                        </tr>
                        <tr>
                            <th class="text-center align-middle">Saat dititipkan
                                <p class="fw-normal m-0">
                                    (<?= $data->rm26iPenyimpananBarang["waktuTitip"] ? date('d-m-Y, H:i', strtotime($data->rm26iPenyimpananBarang["waktuTitip"])) : '-' ?>)
                                </p>
                            </th>
                            <th class="text-center align-middle">Saat diserahkan
                                <p class="fw-normal m-0">
                                    (<?= $data->rm26iPenyimpananBarang["waktuSerah"] ? date('d-m-Y, H:i', strtotime($data->rm26iPenyimpananBarang["waktuSerah"])) : '-' ?>)
                                </p>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php for ($i = 0; $i < count($data->rm26iPenyimpananBarangData); $i++): ?>
                            <tr>
                                <td class="text-center align-middle"><?= $i + 1 ?></td>
                                <td><?= $data->rm26iPenyimpananBarangData[$i]["namaBarang"] ?></td>
                                <td class="text-center align-middle"><?= $data->rm26iPenyimpananBarangData[$i]["jumlah"] ?></td>
                                <td class="text-center align-middle"><?= $data->rm26iPenyimpananBarangData[$i]["kondisiTitip"] ?></td>
                                <td class="text-center align-middle"><?= $data->rm26iPenyimpananBarangData[$i]["kondisiSerah"] ?></td>
                            </tr>
                        <?php endfor; ?>
                    </tbody>
                </table>
                ...
                <div class="row text-center mt-1">
                    <div class="col-12 text-end pe-5">
                        Bangkalan, <?= $data->rm26iPenyimpananBarang['tglTtd'] ?>
                    </div>
                    <table class="table table-borderless">
                        <tr class="text-center" style="margin:auto;">
                            <td>
                                Pasien / Wali Pasien (Yang Menitip)
                                <br><br>

                                <div id="ttdWali">
                                    <?php if ($data->rm26iPenyimpananBarang["ttdWali"]) {
                                        // Sudah ditambahkan 'public/' agar gambar tidak broken/silang
                                        echo '<img src="' . base_url('public/ttd/rm26iPenyimpananBarang/' . $data->rm26iPenyimpananBarang["ttdWali"]) . '" alt="tanda tangan Wali" style="max-width: 150px;" data-is-new="false">';
                                    } else {
                                        echo '<br><br><br><br><br>';
                                    } ?>
                                </div>
                                <br>
                                (<?= $data->rm26iPenyimpananBarang["nama"] ?> )
                                <br><br>
                                <?php if (!$data->rm26iPenyimpananBarang["ttdWali"]) { ?>
                            ...
                            <td>
                                Satpam (Yang Menerima)
                                <br>
                                <br>
                                <div id="qrSatpam" class="pt-2"></div>
                                <br>
                                (<?= $data->rm26iPenyimpananBarang["satpam"] ?> )
                            </td>
                            <td>
                                Petugas
                                <br>
                                ...
                                <br>
                                (<?= $data->rm26iPenyimpananBarang["petugas"] ?> )
                            </td>
                        </tr>
                    </table>
                    <input type="hidden" id="noRawat" value="<?= $data->rm26iPenyimpananBarang["noRawat"] ?>">
                    <input type="hidden" id="petugas" value="<?= $data->rm26iPenyimpananBarang["petugas"] ?>">
                    <input type="hidden" id="satpam" value="<?= $data->rm26iPenyimpananBarang["satpam"] ?>">
                    <div class="row mt-2">
                        <div class="col-12 text-center">
...
<script>
    ...
    var qrSatpam = new QRCode(document.getElementById("qrSatpam"), {
    ...
    qrSatpam.makeCode("Di ttd " + $("#satpam").val() + " untuk Penyimpanan Barang. No Rawat : " + $("#noRawat").val()); // Replace with your desired text or URL
    ...
    function kunciTtd() {
        ...
        $.ajax({
            url: '<?= base_url() ?>rm/rm26iPenyimpananBarang/simpanTtd',
            method: 'post',
            data: {
                noRawat: noRawat,
                ttdWali: ttdWali,
