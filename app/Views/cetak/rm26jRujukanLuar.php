...
<head>
    ...
    <title>Cetak Rujukan Luar</title>
            ...
            <div class="subpage">
                ...
                <div class="row">
                    <div class="col-12 text-center">
                        <p style="font-size: 14pt; margin:10px;" class="text-uppercase fw-bold">FORMULIR
                ...
                <table class="table table-sm table-bordered">
                    <tr>
                        <td>
                            ...
                            <table class="table table-sm ms-2 table-borderless">
                                <tr>
                                    <td>a. Nama pasien</td>
                                    <td>: <?= $data->pasien["nm_pasien"] ?></td>
                                </tr>
                                ...
                                <tr>
                                    <td>c. Umur</td>
                                    <td>: <?= $data->rm26jRujukanLuar["umur"] ?></td>
                                </tr>
                            </table>
                        </td>
                        <td>
                            <b>Asal Rujukan :</b> <?= $data->rm26jRujukanLuar["asal"] ?>
                            <br><br>
                            <b>Alasan Dirujuk :</b> <br>
                            <?= $data->rm26jRujukanLuar["alasan"] ?>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                            <b>II. Diagnosa</b>
                            <div class="ms-3">
                                <?= $data->rm26jRujukanLuar["diagnosa"] ?>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                            <b>III. Keadaan Pasien</b>
                            <table class="table table-sm ms-2 table-borderless">
                                <tr>
                                    <td style="width: 20%;">a. Keadaan umum</td>
                                ...
                                <tr>
                                    <td>c. Tanda-tanda vital</td>
                                    <td>:
                                        <table class="table table-sm">
                                            <tr>
                                                <td>T</td>
                                                <td>: <?= (!empty($data->rm26jRujukanLuar['td_sistol']) ? $data->rm26jRujukanLuar['td_sistol'] : '...') . '/' . (!empty($data->rm26jRujukanLuar['td_diastol']) ? $data->rm26jRujukanLuar['td_diastol'] : '...') ?> mmHg</td>
                                                <td>N</td>
                                                <td>: <?= !empty($data->rm26jRujukanLuar['nadi']) ? $data->rm26jRujukanLuar['nadi'] : '...' ?> x/m</td>
                                                <td>S</td>
                                                <td>: <?= !empty($data->rm26jRujukanLuar['suhu']) ? $data->rm26jRujukanLuar['suhu'] : '...' ?> &deg;C</td>
                                                <td>Rr</td>
                                                <td>: <?= !empty($data->rm26jRujukanLuar['rr']) ? $data->rm26jRujukanLuar['rr'] : '...' ?> x/m</td>
                                            </tr>
                                            <tr>
                                                <td>TFU</td>
                                                <td>: <?= !empty($data->rm26jRujukanLuar['tfu']) ? $data->rm26jRujukanLuar['tfu'] : '...' ?> cm</td>
                                                <td>DJJ</td>
                                                <td>: <?= !empty($data->rm26jRujukanLuar['djj']) ? $data->rm26jRujukanLuar['djj'] : '...' ?> x/m</td>
                                                <td>His</td>
                                                <td>: <?= !empty($data->rm26jRujukanLuar['his']) ? $data->rm26jRujukanLuar['his'] : '...' ?></td>
                                                <td>VT</td>
                                                <td>: <?= !empty($data->rm26jRujukanLuar['vt']) ? $data->rm26jRujukanLuar['vt'] : '...' ?></td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                    ...
                    <tr>
                        <td colspan="2">
                            <b>IV. Penatalaksanaan</b>
                            <table class="table table-sm ms-2 table-borderless">
                                <tr>
                                    <td style="width: 20%;">a. Terapi</td>
                                    <td>: <?= $data->rm26jRujukanLuar['terapi'] ?? '-' ?></td>
                                </tr>
                                <tr>
                                    <td>b. Tindakan</td>
                                    <td>: <?= $data->rm26jRujukanLuar['tindakan'] ?? '-' ?></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                        ...
                        <td colspan="2">
                            ...
                            <div class="ms-3">
                                ...
                                if (!empty($data->rm26jRujukanLuar['handOver'])) {
                                    ...
                                    $items = json_decode($data->rm26jRujukanLuar['handOver'], true);

                                    if (is_array($items) && !empty($items)) {
                                        // Ganti nilai "Lain-Lain" dengan isiHandOverLainLain jika ada
                                        $items = array_map(function ($item) use ($data) {
                                            if ($item === 'Lain-Lain') {
                                                $lainLain = $data->rm26jRujukanLuar['isiHandOverLainLain'] ?? '';
                                                return !empty($lainLain) ? $lainLain : 'Lain-lain';
                                            }
                                            return $item;
                                        }, $items);

                                        // Gabungkan array menjadi string dipisahkan koma
                                        $handOverText = implode(', ', $items);
                                    }
                                }
                                ?>
                            ...
                            <div class="col-12 text-end pe-5">
                                Bangkalan, <?= $data->rm26jRujukanLuar['tglTtd'] ?>
                            </div>
                            <div class="w-100 d-flex justify-content-end pe-5">
                                <div class="w-25 text-center pe-5">
                                    Pengirim
                                    <br><br>
                                    ...
                                    <div id="ttdWali">
                                        <?php if ($data->rm26jRujukanLuar["ttdWali"]) {
                                            ...
                                            echo '<img src="' . base_url('public/ttd/rm26jRujukanLuar/' . $data->rm26jRujukanLuar["ttdWali"]) . '" alt="tanda tangan Wali" style="max-width: 150px;" data-is-new="false">';
                                        } else {
                                            echo '<br><br><br><br><br>';
                                        } ?>
                                    </div>
                                    <br>
                                    (<?= $data->rm26jRujukanLuar["nama"] ?> )
                                    <br><br>
                                    <?php if (!$data->rm26jRujukanLuar["ttdWali"]) { ?>
                    ...
                    <tr>
                        <td colspan="2">
                            <b>VI. Keterangan (diisi oleh Bidan penerima)</b>
                            <div class="ms-3">
                                <?= $data->rm26jRujukanLuar['keteranganBidan'] ?? '-' ?> <br>
                                Alasan : <?= $data->rm26jRujukanLuar['alasanKeterangan'] ?? '-' ?>
                            </div>
                ...
                <div class="row text-center mt-1">
                    ...
                    <input type="hidden" id="noRawat" value="<?= $data->rm26jRujukanLuar["noRawat"] ?>">
                    <input type="hidden" id="petugas" value="<?= $data->rm26jRujukanLuar["petugas"] ?>">
                    <div class="row mt-2">
                        <div class="col-12 text-center">
...
<script>
    ...
    function kunciTtd() {
        ...
        $.ajax({
            url: '<?= base_url() ?>rm/rm26jRujukanLuar/simpanTtd',
            method: 'post',
            data: {
                noRawat: noRawat,
                ttdWali: ttdWali,
