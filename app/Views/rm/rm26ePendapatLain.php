...
/** @var object $data */
if ($data->rm26ePendapatLain) {
    $tglLahir = new \DateTime($data->rm26ePendapatLain["tanggalLahir"]);
}
?>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body" style="overflow-y: auto;">
            <div class="text-center">
                <h5 class="text-uppercase">PERMOHONAN PERMINTAAN PENDAPAT LAIN <br>
                    <i>SECOND OPINION</i>
                </h5>
            ...
            <?php if ($data->rm26ePendapatLain) : ?>
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="alert alert-info">
                            ...
                            <table class="table table-info table-borderless">
                                <tr>
                                    <td>Nama</td>
                                    <td>: <?= $data->rm26ePendapatLain["nama"] . " (" . $data->rm26ePendapatLain["jk"] . ")" ?></td>
                                </tr>
                                <tr>
                                    <td>NIK</td>
                                    <td>: <?= $data->rm26ePendapatLain["nik"]  ?></td>
                                </tr>
                                <tr>
                                    <td>TTL</td>
                                    <td>: <?= $data->rm26ePendapatLain["tempatLahir"] . ", " . $tglLahir->format('d-m-Y') ?></td>
                                </tr>
                                <tr>
                                    <td>Alamat</td>
                                    <td>: <?= $data->rm26ePendapatLain["alamat"] ?></td>
                                </tr>
                                <tr>
                                    <td>Sebagai</td>
                                    <td>: <?= $data->rm26ePendapatLain["sebagai"] ?></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="col-6">
                        <div class="alert alert-info">
                            ...
                            <table class="table table-info table-borderless mb-4">
                                <tr>
                                    <td>Petugas</td>
                                    <td>: <?= $data->rm26ePendapatLain["petugas"] ?></td>
                                </tr>
                                <tr>
                                    <td>Dokter DPJP</td>
                                    <td>: <?= $data->rm26ePendapatLain["dokter"] ?></td>
                                </tr>
                                <tr>
                                    <td>Dokter Lain</td>
                                ...
                                <tr>
                                    <td>Ahli</td>
                                    <td>: <?= $data->rm26ePendapatLain["ahli"] ?></td>
                                </tr>
                                <tr>
                                    <td>Rs</td>
                                    <td>: <?= $data->rm26ePendapatLain["rumahSakit"]  ?></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    ...
                    <br><br>
                    <div class="text-center">
                        <?php if ($data->rm26ePendapatLain['ttdWali']): ?>
                            <a class="btn btn-estetik btn-cetak" href="<?= base_url('/rm/rm26ePendapatLain/cetak/' . str_replace('/', '-', $data->pasien['no_rawat']) . '/' . $data->rm26ePendapatLain['id']) ?>" target="_blank">
                                <i class="fas fa-print me-1"></i> Cetak
                            </a>
                        <?php else: ?>
                            <a class="btn btn-estetik btn-simpan" href="<?= base_url('/rm/rm26ePendapatLain/cetak/' . str_replace('/', '-', $data->pasien['no_rawat']) . '/' . $data->rm26ePendapatLain['id']) ?>" target="_blank">
            ...
            <?php else : ?>
                ...
                <?= $this->include("rm/partials/formRm26ePendapatLain.php") ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-estetik btn-simpan" onclick="simpan(<?= $data->rm26ePendapatLain['id'] ?? '' ?>)">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="datetime-local" class="form-control" id="waktu" value="<?= !empty($data->rm26ePendapatLain) ? date('Y-m-d\TH:i', strtotime($data->rm26ePendapatLain["tglinput"])) : '' ?>">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-estetik btn-batal" data-bs-dismiss="modal"><i class="fas fa-ban me-1"></i> Batal</button>
...
<script>
    ...
    function setSamadgPasien(asal) {
        ...
        if (!$('#samaDgPj').is(':checked') && !$('#samaDgPasien').is(':checked')) {
            $("#nama").val(<?= json_encode($data->rm26ePendapatLain['nama'] ?? '') ?>);
            $("#jk").val(<?= json_encode($data->rm26ePendapatLain['jk'] ?? '') ?>);
            $("#tglLahir").val(<?= json_encode($data->rm26ePendapatLain['tanggalLahir'] ?? '') ?>);
            $("#tempatLahir").val(<?= json_encode($data->rm26ePendapatLain['tempatLahir'] ?? '') ?>);
            $("#alamat").val(<?= json_encode($data->rm26ePendapatLain['alamat'] ?? '') ?>);
            $("#nik").val(<?= json_encode($data->rm26ePendapatLain['nik'] ?? '') ?>);
            $("#sebagai").val(<?= json_encode($data->rm26ePendapatLain['sebagai'] ?? 'Suami') ?>);
    ...
    function simpan(tujuanSimpan) {
        var data = {
            ...
            // Pilihan Dokter & Petugas
            dokter: $('#dokter').val() || '',
            petugas: $('#petugas').val(),

            // Pemberian Informasi
            diagnosa: $('#diagnosa').val(),
            hasilPemeriksaan: $('#hasilPemeriksaan').val(), // Mengambil dari id="alternatif" (Hasil pemeriksaan)

            // Dokter Rumah Sakit Lain
            dokterLain: $('#dokterLain').val(),
            ahli: $('#ahli').val(),
        ...
        if (data.nama.replace(/\s+/g, "-") == "") {
            ...
            $.ajax({
                url: '<?= base_url("rm/rm26ePendapatLain/simpan") ?>',
                method: 'POST',
                data: data,
                dataType: 'json',
    ...
    <?php if ($data->rm26ePendapatLain) : ?>
        ...
        function hapus() {
            var noRawat = "<?= $data->rm26ePendapatLain['noRawat'] ?? '' ?>";

            $.ajax({
                url: '<?= base_url() ?>rm/rm26ePendapatLain/hapus',
                method: 'post',
        ...
        function ubahWaktu() {
            waktu = $("#waktu").val();
            noRawat = "<?= $data->rm26ePendapatLain['noRawat'] ?? '' ?>";

            $.ajax({
                url: '<?= base_url() ?>rm/rm26ePendapatLain/ubahWaktu',
                method: 'post',
                data: {
