    ...
    <div class="card mb-4">
        ...
        <div class="card-body" style="overflow-y: auto;">
            <div class="text-center">
                <h5 class="text-uppercase">PERNYATAAN IZIN KELUAR RUMAH SAKIT SEMENTARA</h5>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <div class="alert alert-info">
                            ...
                            <table class="table table-info table-borderless">
                                <tr>
                                    <td>Nama</td>
                                    <td>: <?= $data->rm26nIzinKeluar["nama"] . " (" . $data->rm26nIzinKeluar["jk"] . ")" ?></td>
                                </tr>
                                <tr>
                                    <td>NIK</td>
                                ...
                                <tr>
                                    <td>Sebagai</td>
                                    <td>: <?= $data->rm26nIzinKeluar["sebagai"] ?></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <div class="alert alert-info">
                            ...
                            <table class="table table-info table-borderless mb-4">
                                <tr>
                                    <td>Petugas</td>
                                    <td>: <?= $data->rm26nIzinKeluar["petugas"] ?></td>
                                </tr>
                                <tr>
                                    <td>Dokter DPJP</td>
                                    <td>: <?= $data->rm26nIzinKeluar["dokter"] ?></td>
                                </tr>
                                <tr>
                                    <td>Alasan</td>
                                ...
                                <tr>
                                    ...
                                    <td>: <?= $data->rm26nIzinKeluar["waktuKembali"] ?></td>
                                </tr>
                                <tr>
                                    <td>No. HP wali</td>
                    ...
                    <div class="text-center">
                        <?php if ($data->rm26nIzinKeluar['ttdWali']): ?>
                            <a class="btn btn-estetik btn-cetak" href="<?= base_url('/rm/rm26nIzinKeluar/cetak/' . str_replace('/', '-', $data->pasien['no_rawat']) . '/' . $data->rm26nIzinKeluar['id']) ?>" target="_blank">
                                <i class="fas fa-print me-1"></i> Cetak
                            </a>
                        <?php else: ?>
                            <a class="btn btn-estetik btn-simpan" href="<?= base_url('/rm/rm26nIzinKeluar/cetak/' . str_replace('/', '-', $data->pasien['no_rawat']) . '/' . $data->rm26nIzinKeluar['id']) ?>" target="_blank">
            ...
            <?php else : ?>
                ...
                <?= $this->include("rm/partials/formRm26nIzinKeluar.php") ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-estetik btn-simpan" onclick="simpan(<?= $data->rm26nIzinKeluar['id'] ?? '' ?>)">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="datetime-local" class="form-control" id="waktu" value="<?= !empty($data->rm26nIzinKeluar) ? date('Y-m-d\TH:i', strtotime($data->rm26nIzinKeluar["tglinput"])) : '' ?>">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-estetik btn-batal" data-bs-dismiss="modal"><i class="fas fa-ban me-1"></i> Batal</button>
...
<script>
    ...
    function setSamadgPasien(asal) {
        if (asal == 'pasien') {
            ...
            $("#noHp").val(<?= json_encode($data->pasien['no_tlp']) ?>);
        ...
        if (!$('#samaDgPj').is(':checked') && !$('#samaDgPasien').is(':checked')) {
            $("#nama").val(<?= json_encode($data->rm26nIzinKeluar['nama'] ?? '') ?>);
            $("#jk").val(<?= json_encode($data->rm26nIzinKeluar['jk'] ?? '') ?>);
            $("#tglLahir").val(<?= json_encode($data->rm26nIzinKeluar['tanggalLahir'] ?? '') ?>);
            $("#tempatLahir").val(<?= json_encode($data->rm26nIzinKeluar['tempatLahir'] ?? '') ?>);
            $("#alamat").val(<?= json_encode($data->rm26nIzinKeluar['alamat'] ?? '') ?>);
            $("#nik").val(<?= json_encode($data->rm26nIzinKeluar['nik'] ?? '') ?>);
            $("#noHp").val(<?= json_encode($data->rm26nIzinKeluar['noHp'] ?? '') ?>);
            $("#sebagai").val(<?= json_encode($data->rm26nIzinKeluar['sebagai'] ?? 'Suami') ?>);
    ...
    function simpan(tujuanSimpan) {
        ...
        if (data.nama.replace(/\s+/g, "-") == "") {
            ...
            $.ajax({
                url: '<?= base_url("rm/rm26nIzinKeluar/simpan") ?>',
                method: 'POST',
                data: data,
                dataType: 'json',
    ...
    <?php if ($data->rm26nIzinKeluar) : ?>
        ...
        function hapus() {
            var noRawat = "<?= $data->rm26nIzinKeluar['noRawat'] ?? '' ?>";

            $.ajax({
                url: '<?= base_url() ?>rm/rm26nIzinKeluar/hapus',
                method: 'post',
        ...
        function ubahWaktu() {
            waktu = $("#waktu").val();
            noRawat = "<?= $data->rm26nIzinKeluar['noRawat'] ?? '' ?>";

            $.ajax({
                url: '<?= base_url() ?>rm/rm26nIzinKeluar/ubahWaktu',
                method: 'post',
                data: {
