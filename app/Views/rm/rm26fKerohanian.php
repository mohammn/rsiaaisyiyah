...
/** @var object $data */
if ($data->rm26fKerohanian) {
    $tglLahir = new \DateTime($data->rm26fKerohanian["tanggalLahir"]);
}
?>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body" style="overflow-y: auto;">
            <div class="text-center">
                <h5 class="text-uppercase">PERMINTAAN PELAYANAN KEROHANIAN</h5>
            ...
            <?php if ($data->rm26fKerohanian) : ?>
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <div class="alert alert-info">
                            ...
                            <table class="table table-info table-borderless">
                                <tr>
                                    <td>Nama</td>
                                    <td>: <?= $data->rm26fKerohanian["nama"] . " (" . $data->rm26fKerohanian["jk"] . ")" ?></td>
                                </tr>
                                <tr>
                                    <td>NIK</td>
                                    <td>: <?= $data->rm26fKerohanian["nik"]  ?></td>
                                </tr>
                                <tr>
                                    <td>TTL</td>
                                    <td>: <?= $data->rm26fKerohanian["tempatLahir"] . ", " . $tglLahir->format('d-m-Y') ?></td>
                                </tr>
                                <tr>
                                    <td>Alamat</td>
                                    <td>: <?= $data->rm26fKerohanian["alamat"] ?></td>
                                </tr>
                                <tr>
                                    <td>Sebagai</td>
                                    <td>: <?= $data->rm26fKerohanian["sebagai"] ?></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <div class="alert alert-info">
                            ...
                            <table class="table table-info table-borderless mb-3">
                                <tr>
                                    <td>Petugas</td>
                                    <td>: <?= $data->rm26fKerohanian["petugas"] ?></td>
                                </tr>
                                <tr>
                                    <td>Petugas Kerohanian</td>
                                    <td>: <?= $data->rm26fKerohanian["petugasKerohanian"] ?></td>
                                </tr>
                                <tr>
                                    <td>Waktu</td>
                                    <td>: <?= $data->rm26fKerohanian["waktu"] ?></td>
                                </tr>
                                <tr>
                                    <td>No. HP</td>
                                    <td>: <?= $data->rm26fKerohanian["noHp"]  ?></td>
                                </tr>
                            </table>
                            <br>
                            <br>
                    ...
                    <br><br>
                    <div class="text-center">
                        <?php if ($data->rm26fKerohanian['ttdWali']): ?>
                            <a class="btn btn-estetik btn-cetak" href="<?= base_url('/rm/rm26fKerohanian/cetak/' . str_replace('/', '-', $data->pasien['no_rawat']) . '/' . $data->rm26fKerohanian['id']) ?>" target="_blank">
                                <i class="fas fa-print me-1"></i> Cetak
                            </a>
                        <?php else: ?>
                            <a class="btn btn-estetik btn-simpan" href="<?= base_url('/rm/rm26fKerohanian/cetak/' . str_replace('/', '-', $data->pasien['no_rawat']) . '/' . $data->rm26fKerohanian['id']) ?>" target="_blank">
            ...
            <?php else : ?>
                ...
                <?= $this->include("rm/partials/formRm26fKerohanian.php") ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-estetik btn-simpan" onclick="simpan(<?= $data->rm26fKerohanian['id'] ?? '' ?>)">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="datetime-local" class="form-control" id="waktu" value="<?= !empty($data->rm26fKerohanian) ? date('Y-m-d\TH:i', strtotime($data->rm26fKerohanian["tglinput"])) : '' ?>">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-estetik btn-batal" data-bs-dismiss="modal"><i class="fas fa-ban me-1"></i> Batal</button>
...
<script>
    ...
    function setSamadgPasien(asal) {
        ...
        if (!$('#samaDgPj').is(':checked') && !$('#samaDgPasien').is(':checked')) {
            $("#nama").val(<?= json_encode($data->rm26fKerohanian['nama'] ?? '') ?>);
            $("#jk").val(<?= json_encode($data->rm26fKerohanian['jk'] ?? '') ?>);
            $("#tglLahir").val(<?= json_encode($data->rm26fKerohanian['tanggalLahir'] ?? '') ?>);
            $("#tempatLahir").val(<?= json_encode($data->rm26fKerohanian['tempatLahir'] ?? '') ?>);
            $("#alamat").val(<?= json_encode($data->rm26fKerohanian['alamat'] ?? '') ?>);
            $("#nik").val(<?= json_encode($data->rm26fKerohanian['nik'] ?? '') ?>);
            $("#sebagai").val(<?= json_encode($data->rm26fKerohanian['sebagai'] ?? 'Suami') ?>);
    ...
    function simpan(tujuanSimpan) {
        var data = {
            ...
            petugasKerohanian: $('#petugasKerohanian').val(), // Berhasil ditangkap secara mandiri sekarang!
            waktu: $('#waktu').val(),
            noHp: $('#noHp').val()
        };
        ...
        if (data.nama.replace(/\s+/g, "-") == "") {
            ...
            $.ajax({
                url: '<?= base_url("rm/rm26fKerohanian/simpan") ?>',
                method: 'POST',
                data: data,
                dataType: 'json',
    ...
    <?php if ($data->rm26fKerohanian) : ?>
        ...
        function hapus() {
            var noRawat = "<?= $data->rm26fKerohanian['noRawat'] ?? '' ?>";

            $.ajax({
                url: '<?= base_url() ?>rm/rm26fKerohanian/hapus',
                method: 'post',
        ...
        function ubahWaktu() {
            waktu = $("#waktu").val();
            noRawat = "<?= $data->rm26fKerohanian['noRawat'] ?? '' ?>";

            $.ajax({
                url: '<?= base_url() ?>rm/rm26fKerohanian/ubahWaktu',
                method: 'post',
                data: {
