...
/** @var object $data */
if ($data->penerjemah) {
    $tglLahir = new \DateTime($data->penerjemah["tanggalLahir"]);
}
?>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body" style="overflow-y: auto;">
            <div class="text-center">
                <h5 class="text-uppercase">FORMULIR PERMINTAAN PENERJEMAH
                </h5>
            ...
            <?php if ($data->penerjemah) : ?>
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="alert alert-info">
                            ...
                            <table class="table table-info table-borderless">
                                <tr>
                                    <td>Nama</td>
                                    <td>: <?= $data->penerjemah["nama"] . " (" . $data->penerjemah["jk"] . ")" ?></td>
                                </tr>
                                <tr>
                                    <td>NIK</td>
                                    <td>: <?= $data->penerjemah["nik"]  ?></td>
                                </tr>
                                <tr>
                                    <td>TTL</td>
                                    <td>: <?= $data->penerjemah["tempatLahir"] . ", " . $tglLahir->format('d-m-Y') ?></td>
                                </tr>
                                <tr>
                                    <td>Alamat</td>
                                    <td>: <?= $data->penerjemah["alamat"] ?></td>
                                </tr>
                                <tr>
                                    <td>Sebagai</td>
                                    <td>: <?= $data->penerjemah["sebagai"] ?></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="col-6">
                        <div class="alert alert-info">
                            ...
                            <table class="table table-info table-borderless mb-4">
                                <tr>
                                    <td>Petugas</td>
                                    <td>: <?= $data->penerjemah["petugas"] ?></td>
                                </tr>
                                <tr>
                                    <td>Bahasa Pasien</td>
                                    <td>: <?= $data->penerjemah["bahasa"] ?></td>
                                </tr>
                                <tr>
                                    <td>No. Hp yg bertanda tangan</td>
                                    <td>: <?= $data->penerjemah["noHp"]  ?></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    ...
                    <br><br>
                    <div class="text-center">
                        <?php if ($data->penerjemah['ttdWali']): ?>
                            <a class="btn btn-estetik btn-cetak" href="<?= base_url('/rm/penerjemah/cetak/' . str_replace('/', '-', $data->pasien['no_rawat']) . '/' . $data->penerjemah['id']) ?>" target="_blank">
                                <i class="fas fa-print me-1"></i> Cetak
                            </a>
                        <?php else: ?>
                            <a class="btn btn-estetik btn-simpan" href="<?= base_url('/rm/penerjemah/cetak/' . str_replace('/', '-', $data->pasien['no_rawat']) . '/' . $data->penerjemah['id']) ?>" target="_blank">
            ...
            <?php else : ?>
                ...
                <?= $this->include("rm/partials/formPenerjemah.php") ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-estetik btn-simpan" onclick="simpan(<?= $data->penerjemah['id'] ?? '' ?>)">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="datetime-local" class="form-control" id="waktu" value="<?= !empty($data->penerjemah) ? date('Y-m-d\TH:i', strtotime($data->penerjemah["tglinput"])) : '' ?>">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-estetik btn-batal" data-bs-dismiss="modal"><i class="fas fa-ban me-1"></i> Batal</button>
...
<script>
    ...
    function setSamadgPasien(asal) {
        ...
        if (!$('#samaDgPj').is(':checked') && !$('#samaDgPasien').is(':checked')) {
            $("#nama").val(<?= json_encode($data->penerjemah['nama'] ?? '') ?>);
            $("#jk").val(<?= json_encode($data->penerjemah['jk'] ?? '') ?>);
            $("#tglLahir").val(<?= json_encode($data->penerjemah['tanggalLahir'] ?? '') ?>);
            $("#tempatLahir").val(<?= json_encode($data->penerjemah['tempatLahir'] ?? '') ?>);
            $("#alamat").val(<?= json_encode($data->penerjemah['alamat'] ?? '') ?>);
            $("#nik").val(<?= json_encode($data->penerjemah['nik'] ?? '') ?>);
            $("#sebagai").val(<?= json_encode($data->penerjemah['sebagai'] ?? 'Suami') ?>);
    ...
    function simpan(tujuanSimpan) {
        ...
        if (data.nama.replace(/\s+/g, "-") == "") {
            ...
            $.ajax({
                url: '<?= base_url("rm/penerjemah/simpan") ?>',
                method: 'POST',
                data: data,
                dataType: 'json',
    ...
    <?php if ($data->penerjemah) : ?>
        ...
        function hapus() {
            var noRawat = "<?= $data->penerjemah['noRawat'] ?? '' ?>";

            $.ajax({
                url: '<?= base_url() ?>rm/penerjemah/hapus',
                method: 'post',
        ...
        function ubahWaktu() {
            waktu = $("#waktu").val();
            noRawat = "<?= $data->penerjemah['noRawat'] ?? '' ?>";

            $.ajax({
                url: '<?= base_url() ?>rm/penerjemah/ubahWaktu',
                method: 'post',
                data: {
