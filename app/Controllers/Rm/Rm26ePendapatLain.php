...
class Rm26ePendapatLain extends BaseController
{
    ...
    public function index($noRawat)
    {
        ...
        $data = (object) [
            ...
            'rm26ePendapatLain' => $rm26ePendapatLain,
            'pjPasien' => $pjPasien,
            'pengaturan' => $pengaturan
        ];

        return view('rm/rm26ePendapatLain', ['data' => $data]);
    }
    ...
    public function simpan()
    {
        ...
        $data = [
            ...
            "tanggalLahir" => $this->request->getPost("tglLahir"), // Menangkap dari JS 'tglLahir' ke kolom 'tanggalLahir'
            "dokter"       => $this->request->getPost("dokter"),
            "nik"          => $this->request->getPost("nik"),

            // Pemberian Informasi
            "diagnosa"     => $this->request->getPost("diagnosa"),
            "hasilPemeriksaan"   => $this->request->getPost("hasilPemeriksaan"),

            // Dokter Rumah Sakit Lain
            "dokterLain"   => $this->request->getPost("dokterLain"),
            "ahli"         => $this->request->getPost("ahli"),
            "rumahSakit"   => $this->request->getPost("rumahSakit"),
        ];

        if ($this->request->getPost("tujuanSimpan") == 'tambah') {
            $this->rm26ePendapatLainModel->save($data);
        } else {
            $noRawat = $this->request->getPost("noRawat");
            unset($data['noRawat']);

            $this->catatLog('ubah', 'ic_sesar', $noRawat, $this->rm26ePendapatLainModel->where('noRawat', $noRawat)->first(), $data);

            $this->rm26ePendapatLainModel->where('noRawat', $noRawat)->set($data)->update();
        }
    ...
    public function hapus()
    {
        $noRawat = $this->request->getPost("noRawat");
        $noRawat = str_replace('-', '/', $noRawat);
        $this->catatLog('hapus', 'ic_darah', $noRawat, $this->rm26ePendapatLainModel->where('noRawat', $noRawat)->first());

        $this->rm26ePendapatLainModel->where("noRawat", $noRawat)->delete();
        echo json_encode("");
    }
    ...
    public function cetak($noRawat)
    {
        ...
        $rm26ePendapatLain = $this->rm26ePendapatLainModel->where('noRawat', $noRawat)->first();
        if ($rm26ePendapatLain) {
            $rm26ePendapatLain["tglTtd"] = $this->tanggalCetak($rm26ePendapatLain["tglinput"]);
        }
        ...
        echo view("cetak/rm26ePendapatLain", ["data" => $data]);
    ...
    public function simpanTtd()
    {
        ...
        $lokasiFolder = 'rm26ePendapatLain';
