...
class Rm26hKepercayaan extends BaseController
{
    ...
    public function index($noRawat)
    {
        ...
        $data = (object) [
            ...
            'rm26hKepercayaan' => $rm26hKepercayaan,
            'pjPasien' => $pjPasien,
            'pengaturan' => $pengaturan
        ];

        return view('rm/rm26hKepercayaan', ['data' => $data]);
    }
    ...
    public function simpan()
    {
        $data = [
            ...
            "nilaiKepercayaan" => $this->request->getPost("nilaiKepercayaan"),
        ];

        if ($this->request->getPost("tujuanSimpan") == 'tambah') {
            $this->rm26hKepercayaanModel->save($data);
        } else {
            $noRawat = $this->request->getPost("noRawat");
            unset($data['noRawat']);

            $this->catatLog('ubah', 'ic_sesar', $noRawat, $this->rm26hKepercayaanModel->where('noRawat', $noRawat)->first(), $data);

            $this->rm26hKepercayaanModel->where('noRawat', $noRawat)->set($data)->update();
        }
    ...
    public function hapus()
    {
        $noRawat = $this->request->getPost("noRawat");
        $noRawat = str_replace('-', '/', $noRawat);
        $this->catatLog('hapus', 'ic_darah', $noRawat, $this->rm26hKepercayaanModel->where('noRawat', $noRawat)->first());

        $this->rm26hKepercayaanModel->where("noRawat", $noRawat)->delete();
        echo json_encode("");
    }
    ...
    public function cetak($noRawat)
    {
        ...
        $rm26hKepercayaan = $this->rm26hKepercayaanModel->where('noRawat', $noRawat)->first();
        if ($rm26hKepercayaan) {
            $rm26hKepercayaan["tglTtd"] = $this->tanggalCetak($rm26hKepercayaan["tglinput"]);
        }
        ...
        echo view("cetak/rm26hKepercayaan", ["data" => $data]);
