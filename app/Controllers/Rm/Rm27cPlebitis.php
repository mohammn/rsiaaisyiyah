...
class Rm27cPlebitis extends BaseController
{
    ...
    public function index($noRawat)
    {
        ...
        $data = (object) [
            ...
            'rm27cPlebitis' => $rm27cPlebitis,
            'pjPasien' => $pjPasien,
            'pengaturan' => $pengaturan
        ];

        return view('rm/rm27cPlebitis', ['data' => $data]);
    }
    ...
    public function simpan()
    {
        ...
        $data = [
            ...
            "lokasiPemasangan"           => json_encode($this->request->getPost("lokasiPemasangan") ?? []),
            "isilokasiPemasanganLainnya" => $this->request->getPost("isilokasiPemasanganLainnya") ?? "",

            "golObat"                    => json_encode($this->request->getPost("golObat") ?? []),
            "isigolObatLainnya"          => $this->request->getPost("isigolObatLainnya") ?? "",

            "ivCath"                     => $this->request->getPost("ivCath") ?? '',
            "isiivCath"                  => $this->request->getPost("isiivCath") ?? '',

            "jenisCairan"                => $this->request->getPost("jenisCairan") ?? '',
        ];
        ...
        if ($this->request->getPost("tujuanSimpan") == 'tambah') {
            $this->rm27cPlebitisModel->save($data);
        } else {
            $noRawat = $this->request->getPost("noRawat");
            unset($data['noRawat']);

            $this->catatLog('ubah', 'rm27c_plebitis', $noRawat, $this->rm27cPlebitisModel->where('noRawat', $noRawat)->first(), $data);

            $this->rm27cPlebitisModel->where('noRawat', $noRawat)->set($data)->update();
        }
    ...
    public function hapus()
    {
        $noRawat = $this->request->getPost("noRawat");
        $noRawat = str_replace('-', '/', $noRawat);
        $this->catatLog('hapus', 'rm27c_plebitis', $noRawat, $this->rm27cPlebitisModel->where('noRawat', $noRawat)->first());

        $this->rm27cPlebitisModel->where("noRawat", $noRawat)->delete();
        echo json_encode("");
    }
    ...
    public function cetak($noRawat)
    {
        ...
        echo view("cetak/rm27cPlebitis", ["data" => $data]);
